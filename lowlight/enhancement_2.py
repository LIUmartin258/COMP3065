...
def simulate_nir(image_path):
    # 打开 RGB 图像
    with Image.open(image_path) as img:
        # 将图像转换为灰度，模拟 NIR 图像的外观
        img_nir = img.convert('L')
        return img_nir

# 图像路径
image_path = 'E:\\COMP3065\\lowlight\\datasets\\LOLv1\\val\\778.png'
# 模拟 NIR 图像
img_nir = simulate_nir(image_path)
# 保存模拟的 NIR 图像
img_nir.save('E:\\COMP3065\\lowlight\\results\\test\\LOLv1\\778_nir.tiff')
...
if __name__ == '__main__':
    img = cv2.imread('E:/FYP/Diffusion-low-light/Diffusion-Low-Light/results/test/LOLv1/778.png')
    nir = np.float32(cv2.imread('E:\\FYP\\Diffusion-low-light\\Diffusion-Low-Light\\results\\test\\LOLv1\\778_nir.tiff')[:, :, 0])
    ...
    cv2.waitKey(0)
    cv2.imwrite('E:/FYP/Diffusion-low-light/Diffusion-Low-Light/results/test/LOLv1/result.tiff', result)